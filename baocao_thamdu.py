...
from database import CoSoDuLieu
...
class Report:
    def __init__(self,root):
        ...
        self.root.iconbitmap('hinhanh\\gaming.ico')  # icon của giao diện
        ...
        img3 = PIL.Image.open(r"hinhanh\bg1.png")#mở ảnh nền
        ...
        #có điểm danh nhưng điểm danh quá muộn giờ buổi học học
        ...
        #student_frame
        student_frame=LabelFrame(Top_frame,bd=1,bg='#A0522D')
        student_frame.place(x=5,y=0,width=315,height=110)

        img_student = PIL.Image.open(r"hinhanh\sv.png")
        # img_student = img_student.resize((50, 50), PIL.Image.ANTIALIAS)
        self.photoimgsv = ImageTk.PhotoImage(img_student)
        student_img = Label(student_frame, image=self.photoimgsv, bg="#A0522D")
        student_img.place(x=20, y=40, width=50, height=50)
        student_text=Label(student_frame,text="Số học sinh",font=("times new roman", 18, "bold"),fg="white",bg="#A0522D")
        student_text.place(x=100,y=30)
        student_text = Label(student_frame,textvariable=self.student, font=("times new roman", 15, "bold"),fg="white",bg="#A0522D")
        student_text.place(x=100, y=70)

        # số bảng điểm danh thành công
        att_frame = LabelFrame(Top_frame, bd=1, bg='#28b779')
        att_frame.place(x=335, y=0, width=315, height=110)

        img_att = PIL.Image.open(r"hinhanh\sodd.png")
        ...
        #chức năng điểm danh đi muộn
        late_frame = LabelFrame(Top_frame, bd=1, bg='#D2691E')
        late_frame.place(x=665, y=0, width=315, height=110)

        img_late = PIL.Image.open(r"hinhanh\late.png")
        # img_student = img_student.resize((50, 50), PIL.Image.ANTIALIAS)
        self.photoimglate = ImageTk.PhotoImage(img_late)
        late_img = Label(late_frame, image=self.photoimglate, bg="#D2691E")
        ...
        #chức năng không điểm danh (nghiệp vụ: thời gian vào và ra quá gần nhau)
        late_frame = LabelFrame(Top_frame, bd=1, bg='#DC143C')
        late_frame.place(x=990, y=0, width=315, height=110)

        img_noatt = PIL.Image.open(r"hinhanh\vang.png")
        # img_student = img_student.resize((50, 50), PIL.Image.ANTIALIAS)
        ...
        #nút tìm kiếm
        img_btn3 = PIL.Image.open(r"hinhanh\btnRed.png")
        ...
        #gán tên cấc cột
        self.LateTable.heading("studentid", text="ID HS")
        ...
        self.NoAttTable.heading("studentid", text="ID HS")
        ...
        self.searchtc_entry.grid(row=0, column=1, padx=5, pady=0, sticky=W)
        #btn tìm
        img_btn2 = PIL.Image.open(r"hinhanh\btnRed.png")
        ...
        self.NotInTable.heading("studentid", text="ID HS")
        ...
        if self.var_com_searchlate.get()=="" or self.var_searchlate.get()=="":
            ...
            try:
                ...
                if(len(data)!=0):
                    ...
                    self.LateTable.delete(*self.LateTable.get_children())
                    messagebox.showinfo("Thông báo", " Không có thông tin thỏa mãn điều kiện tìm kiếm",parent=self.root)
                conn.close()
    ...
    def exportCsv(self):
        try:
            ...
            writer = pd.ExcelWriter(
                'F:\SQLpython\pythonProject\Attendance_CSV\ds_dimuon_' + str(ts) + '_' + str(
    ...
    def exportUnpresetCsv(self):
        try:
            ...
            writer = pd.ExcelWriter(
                'F:\SQLpython\pythonProject\Attendance_CSV\ds_diemdanh_vang_' + str(ts) + '_' + str(
                    self.today) + '.xlsx', engine='xlsxwriter')
    ...
    def exportNotinCsv(self):
        try:
            ...
            writer=pd.ExcelWriter('F:\SQLpython\pythonProject\Attendance_CSV\ds_khong_diemdanh_'+str(ts)+'_'+str(self.today)+'.xlsx',engine='xlsxwriter')
