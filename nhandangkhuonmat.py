...
value_from_home = None# chứa id giảng viên truyền từ form trangchu.py
def new_tcid(value):#hàm lấy thông tin id giáo viên khi đăng nhập vào form trangchu.py
...
class Face_Recognition:
    ...
    def __init__(self,root):
        ...
        self.root.iconbitmap('hinhanh\\gaming.ico')  # icon giao diện
        ...
        img3 = PIL.Image.open(r"hinhanh\bg1.png")#ảnh nền
        ...
        else:# nếu đăng nhập bằng giáo viên
            conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
            my_cursor = conn.cursor()
            my_cursor.execute(
                "SELECT DISTINCT Class,Lesson_Id  from lesson where  Date=%s",
        ...
        #nút camera
        img_btn1 = PIL.Image.open(r"hinhanh\btnOpen.png")#ảnh nút mở camera
        ...
        img_btn2 = PIL.Image.open(r"hinhanh\btnClose.png")#ảnh nút đóng camera
        ...
        # ảnh điểm danh mặc định khi ko có ai điểm danh
        self.img_right = PIL.Image.open(r"hinhanh\unknow.jpg")
        ...
        while True:# khi camera mở lên không có lỗi
            ...
            if(i not in chkarray):
                ...
                if n is not None:
                    ...
                    self.notify_label['text'] = "Thông báo: Học sinh không xác định"
                ...
                try:	
                	#kết nối db
                        ...
                        if(self.type_attendance.get()=="Vào"):
                            if((d1 not in a)) or ((self.lessonid not in b)):# nếu ngày hôm nay học sinh  chưa điểm danh hoặc học sinh chưa điểm danh buổi học dc chọn
                                ...
                                if(time_in<time_start):#nếu thời gian điểm danh vào nhỏ hơn thời gian bắt đầu ->Cập nhật trangh thái thành có mặt
                                    ...
                                    if (b < a < c):#nếu thời gian đi muộn lớn hơn 15 phút và nhỏ hơn 50 phút thì cập nhật trạng thái đi muộn
                                          #Update time tại này ->  nếu phát triển sẽ có thể đánh giá chăm sóc học sinh của trung tâm (chưa làm được)
                                    ...
                                    elif (c < a < d):#nếu thời gian đi muộn lớn hơn 50 và nhỏ hơn 1 tiếng -> vắng
                                        my_cursor.execute("update  attendance set AttendanceStatus=%s where Student_id=%s and Lesson_id=%s",
                                                          ("Bạn đã vắng 1 tiết",str(i),(self.lessonid),))
                        ...
                        elif(self.type_attendance.get()=="Ra"):#Loại điểm danh ra khỏi lớp : tương tự điểm danh vào
                            conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
                            ...
                            if(masp not in att):
                                if ((d1 not in a)) or ((self.lessonid not in b)):
                                    ...
                                    # print(
                                    #     "Học sinh:" + n + " Đã điểm danh ngày " + d1 + ". Vui lòng ra khỏi Camera !!")
                                    self.notify_label[
            ...
            def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):#xác định khuôn mặt trong khung hình
                ...
                for(x,y,w,h) in features:#với mỗi khuôn mặt đã nhận diện đc( x ,y,w,h tạo thành hình vuông khi nhận diện)
                    ...
                    if confidence>85:#Nếu tỉ lệ nhận diện đúng trên 85%
                        cv2.putText(img,f"Ma HS:{i}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),2)#in ra id học sinh
                        cv2.putText(img, f"Ho Ten:{n}", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 2)#in ra tên học sinh
